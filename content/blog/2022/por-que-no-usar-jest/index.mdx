...
En el hilo del tweet hablan de este [issue en Github de serverless](https://github.com/serverless/serverless/pull/6517) donde se comenta las razones por las que acabaron pasando de Jest a Mocha:
  ...
  - Ejecuta los test en un entorno de Node.js copiado / pirateado que presenta algunos problemas:
    - Los *outputs* de `stdout` y `stderr` no se imprimen en el mismo orden en el que se ejecutan.
    - Algunas API públicas de Node.js se reemplazan con alternativas no estándar.
    - Debido al manejo de caché de módulos no estándar y desigual, es propenso a memory leaks.
  - El progreso de los test no se muestra hasta que finalizan, lo cual es inconveniente en las pruebas de larga duración.
...
[Mocha](https://mochajs.org) con [Chai](https://www.chaijs.com) llevan siendo una buena opción para testear tu código en JavaScript desde hace casi 10 años. Es un tandem rápido y ligero: Mocha es el *test runner* y chai es la *assertion library*. Sin embargo, como comentaba antes, el ser ligero hacer que si quieres añadir mocks, spies o informes de cobertura de test necesitas instalar nuevos paquetes.

### Node-Tap ([153,919 descargas semanales](https://www.npmjs.com/package/tap))

[Node Tap](https://node-tap.org) también existe desde hace 10 años, pero no tiene tanta popularidad. Al contrario que Mocha, no es un *test runner*, sino un *test framework* y tal como dice su web: **viene con pilas incluidas**. Al igual que Jest permite mockear módulos, incluye su *assertion library* y poder generar informes de cobertura de test. Es bastante completo, sigue el [protocolo TAP](https://testanything.org) y es el *test framework* que recomienda Matteo Collina. Por cierto, quien creó Node Tap es [Isaac Schlueter](https://twitter.com/izs), el mismo que creó npm. La gran diferencia que con Mocha y Jest es que con Node Tap ejecutas el archivo en el que tienes los test con Node.js directamente, no con el *test runner*. En otras palabras, para ejecutar los test no ejecutas `tap add.test.js`, sino que ejecutas `node add.test.js`.
